...
PARENT_DIR=${1:-"$(readlink -f ${ABS_SCRIPT_DIR}/../ALL_BENCHMARKS-datAFLow-libFuzzer)"}
...
build_target() {
  ...
  RUNDIR="${PARENT_DIR}/RUNDIR-datAFLow_libfuzzer-${TARGET}"
  ...
  # Build target
  export FUZZING_ENGINE="datAFLow_libfuzzer"
  unset CC
  unset CXX
  . ${DIR_NAME}/common.sh
  echo "Running build ${TARGET} (${FUZZING_ENGINE})"
